...
// htmlspecialcharsを短く書くためのヘルパー関数
...
<body>
    ...
    <header class="mt-5">
        <div class="container d-flex justify-content-around align-items-center">
            <h1><a href="index.php"><i class="fa-solid fa-paw"></i>Rate-album</a></h1>
            <nav>
        ...
        <section class="gallery-section">
            ...
            <div class="row g-4">
                <?php if (count($files) > 0): ?>
                    <?php foreach ($files as $file): $f_name = h($file['file_name']); ?>
                        <div class="col-6 col-md-4 col-lg-3">
                            <article class="card h-100 shadow-sm position-relative">
                                <a href="img/<?= $f_name ?>" data-lightbox="latte-album">
                                    <img src="img/<?= $f_name ?>" class="card-img-top gallery-img" alt="ラテの写真" style="height: 200px; object-fit: cover;">
                                </a>
            ...
            <nav class="mt-5" aria-label="ギャラリーページナビゲーション">
                <ul class="pagination justify-content-center">
                    <?php if ($current_page > 1): ?>
                        <li class="page-item"><a class="page-link text-dark" href="gallery.php?page=<?= $current_page - 1 ?>">前へ</a></li>
                    <?php endif; ?>

                    <?php for ($i = 1; $i <= $total_pages; $i++):
                        $isActive = ($i === $current_page); ?>
                        <li class="page-item <?= $isActive ? 'active' : '' ?>">
                            <a class="page-link <?= $isActive ? 'bg-dark border-dark text-white' : 'text-dark' ?>" href="gallery.php?page=<?= $i ?>"><?= $i ?></a>
                        </li>
                    <?php endfor; ?>

                    <?php if ($current_page < $total_pages): ?>
                        <li class="page-item"><a class="page-link text-dark" href="gallery.php?page=<?= $current_page + 1 ?>">次へ</a></li>
                    <?php endif; ?>
                </ul>
            </nav>
        <?php endif; ?>
